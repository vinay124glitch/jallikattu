...
const Bull = ({ position, rotation, isShaking, isCharging = false, speed = 0, actionName = null, actionId = 0, bullState = "IDLE" }: BullProps) => {
  ...
  const smoothedRotation = useRef(rotation);
  ...
  useEffect(() => {
    ...
    const playLocomotion = () => {
      ...
      if (bullState === "CHARGE" || isCharging) {
        ...
        desiredAction = findAction(["RECOVER", "Recover", "Bull_game_rig|Bull_game_walk", "walk"]) || desiredAction;
      } else if (speed > 6) {
  ...
  useEffect(() => {
    ...
    const shot = findAction([actionName, "BACK KICK", "BackKick", "Back_Kick", "Action"]);
    if (!shot) {
  ...
  useFrame((_, delta) => {
    ...
    // Smoothly interpolate rotation to prevent snapping
    let diff = rotation - smoothedRotation.current;
    while (diff > Math.PI) diff -= Math.PI * 2;
    while (diff < -Math.PI) diff += Math.PI * 2;

    smoothedRotation.current += diff * Math.min(delta * 12, 1);
