      ...
      <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-auto">
        <div className="text-center">
          ...
          <div className="mt-8 text-muted-foreground text-sm space-y-1">
            ...
            <p>⚠️ The Bull is aggressive! Don't get caught!</p>
    ...
    return (
      <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-auto">
        <div className="text-center">
          ...
          >
            {timeLeft <= 0 ? "You Won!" : "Caught!"}
          </h2>
          <p className="text-3xl font-bold text-foreground mb-2">
            Survived: <span style={{ color: "hsl(45, 90%, 55%)" }}>{score}s</span>
          </p>
          <p className="text-muted-foreground mb-8">
            {timeLeft <= 0 ? "You outlasted the bull!" : `The bull got you after ${score} seconds.`}
          </p>
          <button
            onClick={onRestart}
  ...
  return (
    <div className="absolute top-0 left-0 right-0 z-10 pointer-events-none p-4">
      <div className="flex justify-between items-start max-w-3xl mx-auto">
        ...
        {/* Timer is redundant now that Time shows score */}

        {/* Attached humans */}
        <div className="bg-card/80 backdrop-blur-sm rounded-xl px-5 py-3 border border-border">
          <div className="text-xs uppercase tracking-widest text-muted-foreground">Caught</div>
          <div className="text-3xl font-black text-primary">
            {attachedCount}/{totalHumans}
          </div>
        </div>
      </div>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex flex-col gap-3 w-full max-w-xl px-4">
        ...
        <div className="flex justify-center gap-8 items-center bg-black/40 backdrop-blur-md rounded-full py-2 px-8 border border-white/10 shadow-2xl">
          ...
          <div className="h-8 w-[1px] bg-white/10" />
          <div className="text-center">
            <div className="text-[9px] uppercase tracking-[0.3em] text-white/50 mb-0.5">Bull Stamina</div>
            <div className="w-24 h-1.5 bg-black/40 rounded-full mt-1 overflow-hidden border border-white/10">
              <div
                className="h-full transition-all duration-300"
                style={{
                  width: `${bullStamina}%`,
                  background: "hsl(18, 72%, 50%)"
                }}
              />
            </div>
          </div>
